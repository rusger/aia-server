...
func TestSubscriptionWindowUpdate(t *testing.T) {
	...
	// "now" sits between past and future so the cases read as their names say.
	...
	cases := []struct {
		...
		{"empty window is backfilled by a live period", "monthly", none, "monthly", &future, "monthly", &future, true},
		{"empty window is NOT backfilled by an expired period", "monthly", none, "monthly", &past, "monthly", nil, false},
		{"empty string window is backfilled", "monthly", stored(""), "yearly", &future, "yearly", &future, true},
		{"empty string window is NOT backfilled by an expired period", "", stored(""), "yearly", &past, "", nil, false},
		...
		{"unreadable stored value NOT replaced by an expired period", "monthly", stored("garbage"), "monthly", &past, "monthly", nil, false},
		{"equal dates keep current", "monthly", stored(future.Format(time.RFC3339)), "yearly", &future, "monthly", &future, false},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			gotLength, gotExpiry, gotAdvance := subscriptionWindowUpdate(c.curLength, c.curExpiry, c.inLength, c.in, now)
